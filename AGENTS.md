...
- Before broad repository inspection, read [`AGENTS_NAVIGATION.md`](AGENTS_NAVIGATION.md) and start
  from its recorded feature route, source-of-truth path, representative symbols, and focused
  validation command.
- Treat the navigation anchors as the default localization path. Reinspect only the smallest
  affected area when a recorded path or symbol is missing, the dependency relation no longer
  matches, compilation/tests contradict the route, or observed behavior differs.
- Do not repeat a whole-repository scan when the navigation entry remains valid.
- When a change adds, moves, or removes a feature entrypoint, public protocol, core owner,
  persistence boundary, primary test location, or nested `AGENTS.md`, update
  `AGENTS_NAVIGATION.md` in the same change.
