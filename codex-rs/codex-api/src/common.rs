...
const EXPLICIT_PROMPT_CACHE_BREAKPOINT_MIN_GPT_MAJOR: u64 = 5;
const EXPLICIT_PROMPT_CACHE_BREAKPOINT_MIN_GPT_MINOR: u64 = 6;

pub(crate) fn add_prompt_cache_breakpoint(mut request: Value) -> Value {
    // Keep cache annotations at the transport boundary so persisted history remains append-only.
    let Some(model) = request.get("model").and_then(Value::as_str) else {
        return request;
    };
    if !supports_explicit_prompt_cache_breakpoints(model) {
        return request;
    }
    ...
    for item in input.iter_mut().rev() {
        ...
        let Some(input_text) = content.iter_mut().rev().find(|content_item| {
            content_item.get("type").and_then(Value::as_str) == Some("input_text")
        }) else {
            continue;
        };
        let Some(input_text) = input_text.as_object_mut() else {
            continue;
        };
        input_text.insert(
...
fn supports_explicit_prompt_cache_breakpoints(model: &str) -> bool {
    ...
    major > EXPLICIT_PROMPT_CACHE_BREAKPOINT_MIN_GPT_MAJOR
        || (major == EXPLICIT_PROMPT_CACHE_BREAKPOINT_MIN_GPT_MAJOR
            && minor >= EXPLICIT_PROMPT_CACHE_BREAKPOINT_MIN_GPT_MINOR)
}
